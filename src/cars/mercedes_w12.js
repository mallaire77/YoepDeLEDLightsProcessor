import { downsizeFrames, downsizeSegments } from './common.js'
...
export const mercedesW12 = (numberOfSegments) => (car) => {
  return {
    ...car,
    LedContainers: car.LedContainers.map(container => {
      // Handle the main conditional container
      if (container.ContainerType === "Groups.CustomConditionalGroup" && container.LedContainers) {
        return {
          ...container,
          LedContainers: container.LedContainers.map(subContainer => {
            // Handle the REV lights group
            if (subContainer.Description === "REV lights") {
              return {
                ...subContainer,
                LedContainers: subContainer.LedContainers.map(gearContainer => {
                  // Process each gear's RPM segments
                  if (gearContainer.ContainerType === "Groups.CustomConditionalGroup" && gearContainer.LedContainers) {
                    return {
                      ...gearContainer,
                      LedContainers: gearContainer.LedContainers.map(rpmContainer => {
                        if (rpmContainer.ContainerType === "RPMSegments") {
                          const downsizedSegments = downsizeSegments(rpmContainer.Segments, numberOfSegments)
                          // Adjust the RPM segments for each gear
                          return {
                            ...rpmContainer,
                            Segments: downsizedSegments,
                            StartPosition: drsLeds,
            ...
            } else if (subContainer.LedContainers && subContainer.LedContainers.some(c => c.ContainerType === "Animation")) {
              return {
                ...subContainer,
                StartPosition: 1,
                LedContainers: subContainer.LedContainers.map(animContainer => {
                  if (animContainer.ContainerType === "Animation") {
                    return {
                      ...animContainer,
                      StartPosition: 1,
                      Animation: {
                        ...animContainer.Animation,
                        Frames: downsizeFrames(animContainer.Animation.Frames, numberOfSegments)
                      }
                    }
                  }
