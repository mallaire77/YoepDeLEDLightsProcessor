...
const preProcessedProfile =
  createMutator(profileSpecificPreProcess)
    ...
    .mutate(paths.openWheelFormulaVeePath, car => ({
      ...car,
      StartPosition: 1
    }))
    .mutate([...paths.openWheelFormulaFordPath, 'LedContainers', { field: 'ContainerType', value: 'Groups.CustomConditionalGroup' }], car => ({
      ...car,
      StartPosition: 1
    }))
    // Add new modules
    .mutate([...paths.leftModulePath, 'LedContainers', { field: 'Description', value: 'Effects' }], container => {
      return {
        ...
        LedContainers: [
          ...
          JSON.parse(fs.readFileSync('./pitlimiter.json', 'utf8'))
        ]
      }
    })
...
if (options.preprocess) {
  ...
  let updatedProfile =
    createMutator(preProcessedProfile)
      ...
      .mutate(paths.variousMx5Path, downsizeCar(middle))
      .mutate(paths.variousGr86Path, downsizeCar(middle))
      .mutate(paths.variousM2Path, downsizeCar(middle))
      .mutate(paths.variousRadicalSR8Path, downsizeCar(middle))
      .mutate(paths.variousRadicalSR10Path, downsizeCar(middle))
      .mutate(paths.variousScaaSpecPath, downsizeCar(middle))
      .mutate(paths.prototypeP499Path, downsizeCar(middle))
      ...
      .mutate(paths.openWheelFormulaVeePath, downsizeCar(middle))
      .mutate(paths.openWheelFormulaFordPath, downsizeCar(middle))
      .mutate(paths.openWheelFormula2000Path, downsizeCar(middle))
      .mutate(paths.openWheelF4Path, downsizeCar(middle))
      .mutate(paths.openWheelF3Path, downsizeCar(middle))
      .mutate(paths.openWheelSuperFormulaLightsPath, downsizeCar(middle))
      .mutate(paths.openWheelSuperFormulaHondaPath, downsizeCar(middle))
      .mutate(paths.openWheelSuperFormulaToyotaPath, downsizeCar(middle))
      .mutate(paths.openWheelMercedesW12Path, mercedesF1(middle))
      .mutate(paths.openWheelMercedesW13Path, mercedesF1(middle))
      .result()
